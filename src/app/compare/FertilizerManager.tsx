...
const FertilizerManager: React.FC = () => {
  ...
  const [savedItems, setSavedItems] = useState<{ _id: string; name: string }[]>(
    []
  )
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/fertilizer/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: saveName, data: periods })
      })
  ...
  const fetchSavedItems = async () => {
    try {
      const response = await fetch('/api/fertilizer/list')
      if (response.ok) {
  ...
  const handleDeleteSavedItem = async (id: string) => {
    try {
      const response = await fetch(`/api/fertilizer/list?id=${id}`, {
        method: 'DELETE'
      })

      if (response.ok) {
        setSavedItems((prev) => prev.filter((item) => item._id !== id)) // Remove item from UI
      } else {
  ...
  const handleLoad = async (id: string) => {
    try {
      const response = await fetch(`/api/fertilizer/load?id=${id}`)
      if (response.ok) {
        const data = await response.json()
  ...
  return (
    <Content style={{ padding: '24px 50px' }}>
      <Col offset={20} style={{ marginBottom: '12px' }}>
        <Space>
        ...
        <Table
          ...
          columns={[
            { title: 'Name', dataIndex: 'name', key: 'name' },
            {
              ...
              render: (_, record) => (
                <Space>
                  <Button type="link" onClick={() => handleLoad(record._id)}>
                    <DownloadOutlined />
                  </Button>
                  <Button
                    type="link"
                    danger
                    onClick={() => handleDeleteSavedItem(record._id)}
                  >
                    <DeleteOutlined />
                  </Button>
                </Space>
